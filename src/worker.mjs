...
const SECURITY_HEADERS = {
  "content-security-policy": "font-src 'self'; frame-src 'self'; img-src 'self' data: https://avatars.githubusercontent.com; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'; object-src 'none'; form-action 'self'; upgrade-insecure-requests",
...
function configurationReady(env) {
  return Boolean(env.DB && env.OAUTH_GITHUB_CLIENT_ID && env.OAUTH_GITHUB_CLIENT_SECRET);
}
...
async function sessionFromRequest(request, env) {
  ...
  const row = await env.DB.prepare(`
    SELECT u.id, u.github_id, u.github_login, u.email, u.name, u.avatar_url,
           u.role, u.status, s.expires_at
...
async function upsertUser(env, identity) {
  ...
  await env.DB.prepare(`
    INSERT INTO users
      (github_id, github_login, email, name, avatar_url, role, status, created_at, updated_at, last_login_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(github_id) DO UPDATE SET
      github_login = excluded.github_login, email = excluded.email, name = excluded.name,
      avatar_url = excluded.avatar_url,
      role = CASE WHEN ? THEN 'admin' ELSE users.role END,
      status = CASE WHEN ? THEN 'approved' ELSE users.status END,
      updated_at = excluded.updated_at, last_login_at = excluded.last_login_at
  `).bind(
    identity.githubId, identity.login, identity.email, identity.name, identity.avatarUrl,
    bootstrapAdmin ? "admin" : "user", bootstrapAdmin ? "approved" : "pending",
    now, now, now, bootstrapAdmin ? 1 : 0, bootstrapAdmin ? 1 : 0,
  ).run();
  return env.DB.prepare(`
    SELECT id, github_id, github_login, email, name, avatar_url, role, status
      FROM users WHERE github_id = ?
  `).bind(identity.githubId).first();
}

function publicUser(user) {
  return {
    id: user.id, githubLogin: user.github_login, email: user.email, name: user.name,
    avatarUrl: user.avatar_url, role: user.role, status: user.status,
  };
}

async function beginGithub(request, env) {
  if (!configurationReady(env)) return redirect("/login/?error=configuration");
...
async function listUsers(env) {
  const result = await env.DB.prepare(`
    SELECT id, github_login, email, name, avatar_url, role, status, created_at, last_login_at
...
async function protectedRoute(request, env, url, area) {
  if (url.pathname === `/${area}`) return redirect(`/${area}/`, 308);
  ...
  if (url.pathname === `/${area}/api/session`) return json({ authenticated: true, user: publicUser(user) });
  ...
  if (url.pathname.startsWith(`/${area}/api/`)) return json({ error: "Not found" }, 404);
