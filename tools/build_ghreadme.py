...
[![Auth]({BADGE}/auth-Cloudflare_Worker-00c2d4?style=for-the-badge&logo=cloudflare&logoColor=white&labelColor=08090c)]({SITE}/login/)
[![Newsletter]({BADGE}/email-Resend_+_D1-e53935?style=for-the-badge&labelColor=08090c)]({SITE}/)

**A Srivan Technologies product for Kubernetes, OpenShift, SRE and platform engineering.**
...
**The public surface explains the system; the member surface protects the
operational paths.** Public pages keep the map, about pages, selected references
and product entry points visible. Signed-in members get the newest issues,
premium topic routes, global search, and the isolated operations path from
Networking through Platform Engineering. Admin, subscribers and newsletter
campaigns run through Cloudflare Workers, D1 and Resend.
...
Platform Ops is no longer just a static newsletter archive. It is a static
knowledge product with a small runtime layer around identity, access and email.

```mermaid
flowchart TB
  VISITOR(["Reader"])
  PUBLIC["Public site<br/>home · about · categories preview · subscribe"]
  LOGIN["/login/<br/>GitHub or Google OAuth"]
  SESSION["Worker session<br/>HttpOnly cookie · SHA-256 token hash"]
  MEMBER["Member surface<br/>latest issues · protected topics · global search"]
  ADMIN["Admin console<br/>users · subscribers · campaigns"]

  D1[("Cloudflare D1<br/>users · sessions · subscribers · deliveries")]
  RESEND["Resend<br/>confirmation and latest issue email"]
  ASSETS[("Static assets<br/>dist/ generated by tools/build.sh")]
  ...
  classDef user fill:#ede9fe,stroke:#6d28d9,color:#4c1d95
  classDef static fill:#e0f2fe,stroke:#0369a1,color:#0c4a6e
  classDef run fill:#fef3c7,stroke:#b45309,color:#78350f
  classDef data fill:#dcfce7,stroke:#15803d,color:#14532d
  class VISITOR user
  class PUBLIC,MEMBER,ASSETS static
  class LOGIN,SESSION,ADMIN,RESEND run
  class D1 data
```

The important boundary: reading public pages still works as static HTML, while
protected issue/topic routes require a signed-in session. Email links route
through `/login/?next=...`, so a reader lands on the protected issue after
authentication rather than on a dead end.
...
- SVG **SMIL** — `<animateMotion>` and friends — is not CSS, so no stylesheet
  ...
  `verify.py` asserts across all {N_PAGES} pages that a page which can animate
  this way can also stop.

`tools/test_motion.py` opens {N_MOTION_PAGES} pages twice, once under each
...
if __name__ == "__main__":
    ...
    for needle, why in (
            ...
            ("## Current product architecture", "the product architecture section"),
    ...
    assert len(blocks) == 4, (
        f"README.md should carry 4 mermaid diagrams (data flow, product runtime, "
        f"end-to-end, integrations); found {len(blocks)}.")
