  ...
  prepare(sql) {
    ...
    return {
      ...
      async first() {
        ...
        if (query.includes("FROM users WHERE github_id")) return db.users.find((item) => item.github_id === values[0]) || null;
      ...
      async run() {
        if (query.startsWith("INSERT INTO sessions")) {
          ...
          const [github_id, github_login, email, name, avatar_url, role, status, created_at, updated_at, last_login_at, forceRole, forceStatus] = values;
          let user = db.users.find((item) => item.github_id === github_id);
          if (!user) {
            user = { id: db.nextId++, github_id, github_login, email, name, avatar_url, role, status, created_at, updated_at, last_login_at };
            db.users.push(user);
          } else {
            Object.assign(user, { github_login, email, name, avatar_url, updated_at, last_login_at });
