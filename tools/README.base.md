...
| 21 | `build_auth.py` | GitHub / Google OAuth + D1 route contract | branded sign-in, signup, `/admin/`, and `/user/` pages; the Worker owns sessions, instant user signup, and Vishal-only admin checks |
| 22 | `build_premium.py` | authenticated-route policy | marks the isolated operations categories noindex before sitemap generation |
...
| 24 | `build_assistant.py` | the finished indexable site + search index | free browser-only troubleshooting retrieval, wired into every page |
| 25 | `build_seo.py` | each page + register | JSON-LD, article dates, per-issue `og:image` |
| 26 | `build_canonical.py` | `siteconf.BASE` | rewrites every origin to the canonical one |
| 27 | `build_headers.py` | every built page, for the origins they fetch from | `_headers` — the security response headers Cloudflare serves. Fails the build if a page starts loading from an origin the policy does not cover |
| 28 | `verify.py` | the built site | `sitemap.xml`, **and a non-zero exit if anything is wrong** |
| 29 | `make_banner.py` | taxonomy + register + commands | `brand/banner.svg` — the animated masthead for the repo's GitHub page |
| 30 | `build_ghreadme.py` | taxonomy + register + commands | `README.md` at the repo root — the repository front page, a different document from the site's own README |
